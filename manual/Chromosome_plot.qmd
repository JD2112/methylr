...
#### Data upload
1. User needs to upload a text (tab-delimited) file with adjusted p-values and logFC values. At present, user can use the DMCs data file directly generated from the main analysis (*methylysis* section @sec-methylysis).
...
::: {.callout-note}
In the Chromosome plot, the \textcolor{blue}{blue} text (e.g. \textcolor{blue}{FAAH}) in the left side is the gene symbol and the \textcolor{orange}{text} in the right side is the CpG ID from Illumina annotation (e.g. \textcolor{orange}{cg20099409}). Please note, CpGs without the gene symbol only shows the CpG ID on the right side.
:::

3. On the right tab, user can also see the volcano data table which is useful when they are using the full dataset from the main analysis (*methylysis* section).

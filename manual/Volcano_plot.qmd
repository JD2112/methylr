...
### Volcano plot
1. The figure will generated as soon as the computation finishes. However, it might takes some more time depending on the size of the file. If user upload a file with 750K rows, it will take 3-5 minutes to generate the figure [See Chapter @sec-calctime]. It is noteworthy that this big data in volcano plot, may be unstable in the browser.
...
3. On the right tab, user can also see the volcano data table which is useful when they are using the full dataset from the main analysis result (*methylysis* @sec-methylysis).
